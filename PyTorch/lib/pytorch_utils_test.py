...
def confmatrix_prec_recall(y,y_pred):
    ''' Compute Confusion Matrix, Precision and Recall for two-class problem - NumPy'''
    n = len(y)
    assert n == len(y_pred)
    xy = y_pred + 2* y
...
class DeepNetTrainer:
            ...
            for i in range(self.last_epoch + 1, self.last_epoch + n_epochs + 1):
                ...
                for phase, is_train in phases:
                    ...
                    for ii, (X, Y) in enumerate(data[phase]):
                        ...
                        thisloss = loss.data.cpu().numpy()
                        epo_loss += thisloss
                        epo_samp += 1

                        #print(Ypred.data.cpu().mean())
                        #print(self.model.state_dict()['bn1.weight'].mean())
                        #print('is_train,fase,ii,loss',is_train,phase, ii, thisloss, epo_loss)
    ...
    def predict(self, data_loader, use_gpu='auto', generate_labels=False):
        if use_gpu == 'auto':
            use_gpu = torch.cuda.is_available()
        predictions = []
        labels = []
    ...
    def evaluate_confusion_matrix(self, data_loader, use_gpu='auto'):
        logits, labels = self.predict(data_loader, use_gpu, generate_labels=True)
        cm,p,r = compute_confmatrix_prec_recall(logits, labels)
        return cm

    ##### Fim da adição por RAL 
